...
def download_dataset_qm9(datadir, dataname, splits=None, calculate_thermo=True, exclude=True, cleanup=True):
    ...
    for split, split_idx in splits.items():
        gdb9_data[split], smiles[split] = process_xyz_files(
    ...
    for split, data in smiles.items():
        save_smile_dir = join(gdb9dir, split+'_smiles.npy')
        np.save(save_smile_dir,data)
        # np.savez_compressed(save_smile_dir, **data)
