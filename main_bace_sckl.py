...
from qm9.models import EGNN, GNN, PredictionKL_C, SSL_3d, SSL_2d
from leaky_mib import Solver
from data_scaffold_bace import GetData, GetValData, GetTestData
...
parser.add_argument('--exp_name', type=str, default='kfold', metavar='N',
...
parser.add_argument('--weight_decay', type=str, default=1e-18, metavar='N',
                    help='weight decay')
parser.add_argument('--outf', type=str, default='bace/logs', metavar='N',
...
dataloaders_valid = DataLoader(data_valid, batch_size = 10, shuffle = True, drop_last=False, num_workers=2)
...
prediction = PredictionKL_C(in_node_nf=73, in_nf=2*args.nf, hidden_nf=args.nf, device=device,
                 n_layers=args.n_layers, coords_weight=1.0, attention=args.attention, node_attr=args.node_attr)
...
def train(epoch, loader, partition='train'):
    ...
    for batch, data in enumerate(loader):
        ...
        labels = trans_nan(data["labels"][:,0]).to(device).long()

        one_label_flag = only_one_label(labels)
        if one_label_flag: continue
        ...
        # atom_positions_2d = torch.zeros_like(atom_positions)
        ...
        if partition == 'train':
            loss = loss_ce(pred, labels)
            loss_ssl_3d = loss_ce(ration_pre.view(-1,3), rationality.view(-1,1).squeeze(1))
            loss_ssl_2d = loss_mse(distance_pre, distance)
            if epoch % 10 == 0:
                print(loss.item() , args.alpha*loss_ssl_2d.item() , args.beta*loss_ssl_3d.item(), kl_loss.item())
            loss_sum = loss + args.alpha*loss_ssl_3d + args.beta*loss_ssl_2d + kl_loss
            loss_sum.backward()
            optimizer.step()
        else:
            loss = loss_ce(pred, labels)
        ...
        try:
            ...
            res['loss'] += roc_auc * batch_size
            res['counter'] += batch_size
            res['loss_arr'].append(roc_auc)

            prefix = ""
            if partition != 'train':
                prefix = ">> %s \t" % partition

            print(prefix + "Epoch %d \t Iteration %d \t auc %.4f \t knn %d \t lr %f \t beta %f \t fold %d" % (epoch, batch, sum(res['loss_arr'][-10:])/len(res['loss_arr'][-10:]), args.knn, args.alpha, args.beta, args.fold))
        except ValueError:
            print('Only one label in this batch, pass')
            pass
...
if __name__ == "__main__":
    ...
    best_result = 0
    res = {'epochs': [], 'train_loss': [], 'val_loss': [], 'best_val': 0, 'best_test': 0, 'best_epoch': 0}
    ...
    # if res['best_val'] > best_result:
    #     best_result = res['best_val']
    #     files = glob.glob('*_bace_knn.pkl')
    #     for file in files:
    #         os.remove(file)
    #     torch.save({'model': model.state_dict(),
    #             'model_2d': model_2d.state_dict(),
    #             'prediction': prediction.state_dict(),
    #             'optimizer':optimizer.state_dict(),
    #             'ssl_2d':ssl_2d.state_dict(),
    #             'ssl_3d':ssl_3d.state_dict(),
    #             'epoch': epoch
    #             }, '{}_{}_{}_bace_knn.pkl'.format(args.fold, args.knn, epoch))
